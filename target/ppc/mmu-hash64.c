...
void helper_SLBMTE(CPUPPCState *env, target_ulong rb, target_ulong rs)
{
    ...
    /*
     * Decode RB the way hardware does.  The SLBE index sits in bits 52:63,
     * but an implementation only looks at as many of those bits as it has
     * entries for, and bits 37:51 are reserved and ignored outright.
     *
    ...
    assert(is_power_of_2(cpu->hash64_opts->slb_size));
